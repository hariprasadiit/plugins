...
previewPhotoSampleBuffer:(CMSampleBufferRef)previewPhotoSampleBuffer
    ...
    UIImage *rotated = fixOrientationOfImage(image);
...
@property(readonly, nonatomic) AVCaptureOutput *capturePhotoOutput;
...
- (instancetype)initWithCameraName:(NSString *)cameraName
    ...
    if (@available(iOS 10.0, *)) {
        ...
        [((AVCapturePhotoOutput *)_capturePhotoOutput) setHighResolutionCaptureEnabled:YES];
    } else {
        _capturePhotoOutput = [AVCaptureStillImageOutput new];
...
- (void)captureToFile:(NSString *)path result:(FlutterResult)result {
    if (@available(iOS 10.0, *)) {
        ...
        AVCaptureStillImageOutput *captureStillImageOutput = (AVCaptureStillImageOutput *)_capturePhotoOutput;
        captureStillImageOutput.outputSettings = @{AVVideoCodecKey:AVVideoCodecJPEG};
        
        if(captureStillImageOutput.stillImageStabilizationSupported){
            captureStillImageOutput.automaticallyEnablesStillImageStabilizationWhenAvailable = YES;
        }
        
        AVCaptureConnection *connection = [captureStillImageOutput connectionWithMediaType:AVMediaTypeVideo];
        
        [captureStillImageOutput captureStillImageAsynchronouslyFromConnection:connection completionHandler:^(CMSampleBufferRef  _Nullable sampleBuffer, NSError * _Nullable error) {
            ...
            float yxAtan = (atan2(self->_motionManager.accelerometerData.acceleration.y,
                                  self->_motionManager.accelerometerData.acceleration.x)) *
            180 / M_PI;
            
            UIImageOrientation orienation;
            
            if (isNearValue(-90.0, yxAtan)) {
                orienation = UIImageOrientationRight;
            } else if (isNearValueABS(180.0, yxAtan)) {
                orienation = self->_captureDevice.position == AVCaptureDevicePositionBack ? UIImageOrientationUp
                : UIImageOrientationDown;
            } else if (isNearValueABS(0.0, yxAtan)) {
                orienation = self->_captureDevice.position == AVCaptureDevicePositionBack ? UIImageOrientationDown /*rotate 180* */
                : UIImageOrientationUp /*do not rotate*/;
            } else if (isNearValue(90.0, yxAtan)) {
                orienation = UIImageOrientationLeft;
            }
            // If none of the above, then the device is likely facing straight down or straight up -- just
            // pick something arbitrary
            // TODO: Maybe use the UIInterfaceOrientation if in these scenarios
